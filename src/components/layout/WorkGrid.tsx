...
// ── Grid-view card ────────────────────────────────────────────────────────────
function Card({ project, reduce }: { project: Project; reduce: boolean | null }) {
  return (
    <Link
      to="/contact"
...
export default function WorkGrid() {
  ...
  const [hovered, setHovered] = useState<Project | null>(null) // project for the list-view cursor preview
  const [activeIndex, setActiveIndex] = useState(-1) // list-view scroll-active row (mobile)
  const previewRef = useRef<HTMLDivElement>(null)
  const sectionRef = useRef<HTMLElement>(null)
  const listRowsRef = useRef<(HTMLAnchorElement | null)[]>([])
  ...
  const onListMove = (e: MouseEvent<HTMLDivElement>) => {
